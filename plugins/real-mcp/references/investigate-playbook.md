...
| Ask | Use | Do not |
|---|---|---|
| 誰最近找過／收件匣 | `inbox_list`（`real-inbox`） | treat as full DB census |
...
7. Do not blind-retry the same failing query. On auth errors → `real-session`.
