...
description: Search REAL inbox message bodies with messages_search (read-only, max 30-day window). Load investigate-playbook for sample/FAQ rules.
---
...
| Use when | Do not use when |
|---|---|
| 關鍵字／客訴主題／FAQ 草稿證據 | 「誰找過」→ `real-inbox` |
...
5. After a hit, optional `real-inbox` → `conversation_get` for recent context.
6. Reply drafting → `real-messaging`. FAQ draft → playbook thin path + optional `knowledge_search`.
