# Error recovery (INFINITY LABS / `infinity-labs`)
...
| Agent | Action |
|---|---|
| Cursor | Plugin → MCPs → `infinity-labs` → Logout（若有）→ Authenticate |
...
The project allowlist disabled that tool. Tell the user it is turned off for this project in the INFINITY LABS MCP settings. Do **not** invent a substitute tool name.
