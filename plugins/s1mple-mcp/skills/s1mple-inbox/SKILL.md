---
name: s1mple-inbox
description: Browse s1mple Unified Inbox threads (inbox_list, conversation_get). Read-only message-center triage — not period corpus search.
---
...
| Use when | Do not use when |
|---|---|
| 誰最近找過、收件匣分流 | 情緒占比／期間主題統計 → `s1mple-investigate` |
...
3. For keyword / period evidence across the project, hand off to `s1mple-investigate` (`messages_search`).

## Guardrails

- Read-only. Sending → `s1mple-messaging` (preview → propose → approve).
