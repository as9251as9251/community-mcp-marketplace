...
| Ask | Use | Do not |
|---|---|---|
| 誰最近找過／收件匣 | `inbox_list`（`s1mple-inbox`） | treat as full DB census |
...
7. Do not blind-retry the same failing query. On auth errors → `s1mple-session`.
